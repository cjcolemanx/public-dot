local api = vim.api
local fn = vim.fn
local escape = 27

local M = {}

local hint_keys = { "1", "2", "3", "4", "5", "6", "7", "8", "9" }

-- Returns a table that maps the hint keys to their corresponding windows.
local function get_window_keys(windows)
  local mapping = {}
  local chars = hint_keys
  local nrs = {}
  local ids = {}
  local current = api.nvim_win_get_number(api.nvim_get_current_win())

  -- We use the window number (not the ID) as these are more consistent. This in
  -- turn should result in a more consistent choice of window keys.
  for _, win in ipairs(windows) do
    local nr = api.nvim_win_get_number(win)

    table.insert(nrs, nr)
    ids[nr] = win
  end

  table.sort(nrs)

  local index = 1

  for _, nr in ipairs(nrs) do
    -- We skip the current window here so that we still "reserve" it the
    -- character, but don't include it in the output. This ensures that window X
    -- always gets hint Y, regardless of what the current active window is.
    if nr ~= current then
      local key = chars[index]

      if mapping[key] then
        key = key .. (index == #chars and chars[1] or chars[index + 1])
      end

      mapping[key] = ids[nr]
    end

    index = index == #chars and 1 or index + 1
  end

  return mapping
end

local function draw_hint_windows()
  local windows = api.nvim_tabpage_list_wins(0)
  local hint_popups = {}

  for key, win in ipairs(windows) do
    local bufnr = api.nvim_create_buf(false, true)
    local float_height = 3
    local float_width = 6

    if bufnr > 0 then
      local win_width = api.nvim_win_get_width(win)
      local win_height = api.nvim_win_get_height(win)

      local row = math.max(0, math.floor((win_height / 2) - 1))
      local col = math.max(0, math.floor((win_width / 2) - float_width))

      api.nvim_buf_set_lines(bufnr, 0, -1, true, { "", "  " .. key .. "  ", "" })
      api.nvim_buf_add_highlight(bufnr, 0, "Normal", 1, 0, -1)

      local float_window = api.nvim_open_win(bufnr, false, {
        relative = "win",
        win = win,
        row = row,
        col = col,
        -- width = #key == 1 and float_width - 1 or float_width,
        width = float_width,
        height = float_height,
        focusable = false,
        style = "minimal",
        border = "single",
        noautocmd = true,
      })

      api.nvim_win_set_option(float_window, "winhl", "Normal:" .. "Bold")
      api.nvim_win_set_option(float_window, "diff", false)

      hint_popups[float_window] = bufnr
    end
  end

  vim.cmd("redraw")

  -- print(vim.inspect(hint_popups))
  return hint_popups
end

local function close_hints(hint_popups)
  for window, bufnr in pairs(hint_popups) do
    api.nvim_win_close(window, true)
    api.nvim_buf_delete(bufnr, { force = true })
  end
end

local function get_char()
  local ok, char = pcall(fn.getchar)
  return ok and fn.nr2char(char) or nil
end

function M.window_switcher()
  local windows = vim.tbl_filter(function(id)
    return api.nvim_win_get_config(id).relative == ""
  end, api.nvim_tabpage_list_wins(0))

  local window_keys = get_window_keys(windows)
  local floats = draw_hint_windows()
  local key = get_char()
  local window = nil

  if not key or key == escape then
    close_hints(floats)
    return
  end

  window = window_keys[key]
  local extra = {}
  local choices = 0

  for hint, win in pairs(hint_keys) do
    if vim.startswith("" .. hint .. "", key) then
      extra[hint] = win
      -- window = hint
      choices = choices + 1
    end
  end

  if choices > 1 then
    close_hints(floats)

    floats = draw_hint_windows()

    local second = get_char()

    if second then
      local combined = key .. second

      window = hint_keys[combined] or hint_keys[key]
    else
      window = nil
    end
  end

  close_hints(floats)

  if window then
    api.nvim_set_current_win(window)
  end
end

return M
